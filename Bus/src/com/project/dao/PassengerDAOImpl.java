...
public class PassengerDAOImpl implements PassengerDAO{
	...
	public int searchPassenger(String username) {
		int passengerID=0;
		try(Connection con = DBUtils.provideConnection()) {
			String query="SELECT passengerID FROM Passenger where userName = ?";
	...
	public void updatePassenger(PassengerDTO passenger) {
		try(Connection con = DBUtils.provideConnection()) {
			String query="UPDATE  Passenger SET Name = ?, mobileNumber = ?, Password = ? where userName = ?";
			PreparedStatement ps = con.prepareStatement(query);
			...
			if(ps.executeUpdate()>0) {
				System.out.println("HURRAY!,  passenger updated successfully.");
				
			}
			else {
				System.out.println("sorry kuch galat he");
			}
