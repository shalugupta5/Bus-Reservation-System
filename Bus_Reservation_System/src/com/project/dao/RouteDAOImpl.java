...
public class RouteDAOImpl implements RouteDAO{
	...
	public void addRoute(RouteDTO route) {
		try(Connection con = DBUtils.provideConnection()){
			...
			if(ps.executeUpdate()>0) {
				System.out.println(ConsoleColors.GREEN_BOLD+"New route inserted successfully");
			}
		}catch(Exception ex) {
	...
	public void updateRoute(RouteDTO route) {
			...
			String query="UPDATE Routes SET SourceLocation = ?, DestinationLocation = ?, Distance = ? WHERE RouteID = ? and is_deleted = 0";
			PreparedStatement ps = con.prepareStatement(query);
			ps.setString(1, route.getSourceLocation());
			ps.setString(2, route.getDestinationLocation());
			ps.setInt(3, route.getDistance());
			...
			if(ps.executeUpdate()>0) {
				System.out.println(ConsoleColors.GREEN_BOLD+"New route updated successfully");
			}
		}catch(Exception ex) {
			System.out.println(ConsoleColors.RED_BOLD+ex.getMessage());
		}
	...
	public void deleteRoute(int routeID) {
		try(Connection con = DBUtils.provideConnection()){
			...
			if(ps.executeUpdate()>0) {
				System.out.println(ConsoleColors.GREEN_BOLD+"Route record deleted successfully");
			}
